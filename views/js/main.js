...
// Changes the value for the size of the pizza above the slider --- Moved this into the global scope so it was not created on slider change.
...
// Changes the slider value to a percent width --- Moved this into the global scope so it was not created on slider change.
...
// Returns the size difference to change a pizza element from one size to another. Called by changePizzaSlices(size). --- Moved this into the global scope so it was not created on slider change.
...
// Iterates through pizza elements on the page and changes their widths --- Moved this into the global scope so it was not created on slider change.
function changePizzaSizes(size) {
  // Created a container veriable and moved the dx variable to store the container elements and dx veriable only once per call. 
  var container = document.getElementsByClassName("randomPizzaContainer");
  var dx = determineDx(container[0], size); // Only need to use container[0] as all the containers are the same size.
...
//Changed all queryselectorsall to be getElementsByClassName as this performs much faster.
var items = document.getElementsByClassName('mover');

function updatePositions() {
  // Created scrollTop to store this variable outside the loop. Created phaseValues to store the array of phases required.
  var scrollTop = document.body.scrollTop;
  var phaseValues = [];
  frame++;
  window.performance.mark("mark_start_frame");
  //Only need 5 values as they are the only distinct values generated for the items.
  for (i = 0; i < 5; i++) {
    phaseValues.push(Math.sin((scrollTop / 1250) + i));
  }
  for (var i = 0, length = items.length; i < length; i++) {
    var value = phaseValues[i % 5];
    items[i].style.left = items[i].basicLeft + 100 * value + 'px';
  }
...
document.addEventListener('DOMContentLoaded', function () {
  var cols = 8;
  var s = 256;
  //reduced the number of items created to be about 40 from 200 as this is more than what is shown on page at one time.
  for (var i = 0; i < 40; i++) {
